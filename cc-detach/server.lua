RegisterServerEvent('fivem_detach:detachToCall')
AddEventHandler('fivem_detach:detachToCall', function(callId)
...
function AttachToCall(callId, discordId, playerId)
    ...
    PerformHttpRequest(url, function(statusCode, response, headers)
        if statusCode == 200 then
            TriggerClientEvent('chat:addMessage', playerId, { args = { '^2SYSTEM', 'You have been successfully been detached from the call.' } })
        else
            TriggerClientEvent('chat:addMessage', playerId, { args = { '^1SYSTEM', 'Failed to detach to the call. Please try again.' } })
        end
